...
├── index.tsx           # 应用入口
├── manifest.json       # 应用清单
├── routePrefetch.ts    # 应用路由预加载
├── components/         # 主数据专用组件
├── constants/          # 常量
├── hooks/              # 专用 Hooks
├── pages/              # 业务页面
├── schemas/            # 表单/查询 Schema
├── services/           # /apps/master-data API
├── types/              # 业务类型
└── utils/              # 业务工具
```
...
- 后端前缀：`/apps/master-data`，同时被快PLM等应用通过 `master-data-links` 引用。
- 数据字典：标准化编码规则。
- 数据质量：唯一性与格式校验。
