...
详情组件兼容导出层。`UniDetail` 与 `layout-templates` 中的 `DetailDrawerTemplate` 是同一实现（别名）。
...
| 符号 | 说明 |
|------|------|
| `UniDetail` | `DetailDrawerTemplate` 的别名 |
| `detailDrawerDescriptionItems` | 详情描述项工具 |
| `UniDetailProps` | 组件 Props 类型 |

## 约定

- 修改详情实现时改 `components/layout-templates`，不要在此目录复制实现。
- 已使用 `UniDetail` 的地方保持导出路径稳定。
